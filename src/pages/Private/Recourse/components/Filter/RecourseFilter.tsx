...
const RecourseFilter = () => {
  ...
  const getFilterData = async (val) => {
    return await fetch(`${import.meta.env.VITE_BACKEND_ENDPOINT}/settings/${val}`)
  ...
  const execFilter = () => {
    ...
    if (!!searchNombre && searchNombre !== '') searchParams.append('searchNombre', searchNombre);

    if (!!searchTipo && searchTipo !== '0') searchParams.append('searchTipo', searchTipo);

    if (!!searchEstado && searchEstado !== '0') searchParams.append('searchEstado', searchEstado);
